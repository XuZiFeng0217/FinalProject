 ...
 * @author zzz
 */
public class Z33 extends javax.swing.JPanel {
    ...
    private int selectedPieIndex = -1; // 鼠标点击是选中的Arc, -1为没有选中
     ...
     public Z33(JPanel userProcessContainer, UserAccount account, EcoSystem business) {
        initComponents();
        ...
        if(account.getUsername().equals("wd1")){
        ...
        if(account.getUsername().equals("wd3")){
            for(Order order : business.getOrderDirectory()){
                if(order.getWareHouse().getName().equals("wh3")){
                    if(order.getCompany().getName().equals("SF")){
                        ad1 ++ ;
                    }else if(order.getCompany().getName().equals("EXPRESS")){
                        ad2++;
